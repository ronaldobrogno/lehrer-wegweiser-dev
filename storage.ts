...
export function getSeatingPlan(classId: string): SeatingPlan | null {
  try {
    ...
    const rawPlan = all[classId];

    if (!rawPlan) return null;

    const normalizedPlan: SeatingPlan = {
      ...rawPlan,
      seats: (rawPlan.seats || []).map((seat: any) => ({
        id: seat.id,
        x: seat.x,
        y: seat.y,
        label: seat.label,
        studentIds: Array.isArray(seat.studentIds)
          ? seat.studentIds
          : seat.studentId
          ? [seat.studentId]
          : [],
...
export function getElternabende(): Elternabend[] {
  try {
    return JSON.parse(localStorage.getItem(ELTERNABEND_KEY) || "[]");
  } catch {
    return [];
  }
}
